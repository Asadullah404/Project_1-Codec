...
class FfmpegCodec:
    ...
    def process(self, original_wav_data):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                sf.write(orig_file, original_wav_data, SR)
                ...
                # 5. Build FFMPEG command for decoding
                decode_cmd = [
                ...
                # 7. Load the reconstructed .wav file
...
class OpusCodec(FfmpegCodec):
    def __init__(self):
        # Using libopus, 12kbps, opus format
        super().__init__(codec_name="libopus", bitrate_kbps=12, audio_format="opus")

class AacCodec(FfmpegCodec):
    def __init__(self):
        # Using built-in aac, 12kbps, aac format
        super().__init__(codec_name="aac", bitrate_kbps=12, audio_format="aac")
...
def get_model_and_meta(model_name, model_path):
    ...
    try:
        if "Uncompressed" in model_name:
            ...
            bitrate = "~12.65 kbps"
            latency = "20ms"
        elif "Opus" in model_name: # *** NEW ***
            if not is_ffmpeg_available():
                return None, "N/A", "N/A", "ffmpeg not found. Please install ffmpeg."
            model = OpusCodec()
            bitrate = "~12.00 kbps"
            latency = "File-based" # Latency is not comparable in file mode
        elif "AAC" in model_name: # *** NEW ***
            if not is_ffmpeg_available():
                return None, "N/A", "N/A", "ffmpeg not found. Please install ffmpeg."
            model = AacCodec()
            bitrate = "~12.00 kbps"
...
class AnalysisWorker(QObject):
    ...
    def __init__(self, audio_file_path, tiny_model_path):
        ...
        self.models_to_test = [
            ...
            "AMR-WB (Simulated)", 
            "Opus (12 kbps, ffmpeg)", # *** NEW ***
            "AAC (12 kbps, ffmpeg)",  # *** NEW ***
            "DAC Codec (16kHz)",
            ...
            try:
                ...
                with torch.no_grad():
                    if model is not None:
                        # *** NEW LOGIC: Separate Streaming vs. File-based models ***
                        
                        if isinstance(model, (OpusCodec, AacCodec)):
                            # --- 1. FILE-BASED EVALUATION (Opus, AAC) ---
                            # These models process the *entire* file at once.
                            reconstructed_wav = model.process(original_wav)
                        
                        else:
                            # --- 2. STREAMING (OaS) EVALUATION (Tiny, DAC, MuLaw, etc.) ---
                            # This is the original, correct OaS logic.
...
class DetailedAnalysisTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        # *** MODIFIED: Added check for ffmpeg ***
        status_text = "Status: Ready."
        if not is_ffmpeg_available():
            status_text += " (WARNING: ffmpeg not found. Opus/AAC comparison will be skipped.)"
    ...
    def run_analysis(self):
        ...
        if not is_ffmpeg_available():
            self.status_label.setText("Status: Starting... (Note: ffmpeg not found, skipping Opus/AAC)")
    ...
    def display_results(self, results):
        ...
        for row, data in enumerate(results):
            ...
            # *** NEW: Handle file-based latency display ***
            lat_text = latency
            if "File-based" in latency:
                lat_text = "--" # Don't show a number
                if rtf != "--": # But show processing time
                    rtf_text = f"{rtf} (File-based)"
    ...
    def plot_results(self, results):
        ...
        # *** NEW: Handle file-based display ***
        bar_labels = df['name']
        for i, name in enumerate(df['name']):
            if "Opus" in name or "AAC" in name:
                colors[i] = 'gray' # Color file-based codecs differently
    ...
    def export_results(self):
        ...
        try:
            with open(filepath, 'w') as f:
                ...
                f.write("\nOpus/AAC are tested in file-based mode using ffmpeg.")
