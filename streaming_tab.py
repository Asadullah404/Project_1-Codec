...
# BROADCAST_PORT = 37020
# STREAM_PORT = 37021
# CONTROL_PORT = 37022 # New port for handshake messages
# FORMAT = pyaudio.paInt16
# CHANNELS = 1
# RATE = 16000
# CHUNK = HOP_SIZE  # 15ms = 240 samples
...
#                         # This is a response to our broadcast
#                         hostname = message.split(':', 1)[1]
#                         self.peer_found.emit(hostname, addr[0])
...
#     """Listens for handshake messages (request, accept, decline, stop)."""
...
#         self.pending_stream_request_ip = None # IP we sent a request to
        
#         self._setup_ui()
#         self._start_discovery()
#         self._start_control_listener() # Start the new control listener
#         self.model = None
...
#         # Don't show popup if already streaming
...
#         # Handle responses to a request we sent
...
#                 # They accepted, now we start our worker to send audio
#                 self.start_streaming_worker(ip)
...
#         # Handle messages from an active stream
...
#                 self.stop_streaming() # Stop our end
...
#         self.connect_button.setEnabled(False)
#         self.disconnect_button.setEnabled(True) # Allow user to cancel request
...
#     def stop_streaming(self):
#         # If we were waiting for a request, cancel it
...
#         # If we are in an active stream, stop it and notify peer
#         if self.streamer_worker:
        ...
        with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
            ...
            while self._running:
                try:
                    ...
                    continue
                # --- START FIX 1: Ignore common non-fatal socket errors ---
                except socket.error as e:
                    # This ignores errors like [WinError 10054] which are
                    # common with UDP and firewalls, but not fatal.
...
class StreamerWorker(QObject):
    ...
    def __init__(self, target_ip, model, model_type_str):
        ...
        self.model = model
        self.model_type_str = model_type_str
        # --- START FIX: Use separate PyAudio instances for input and output ---
    ...
    def send_audio(self):
        # --- START FIX: Use instance variables ---
        self.input_stream = None
        try:
            # Use CHUNK (240 samples = 15ms) for low-latency input buffer
            ...
            with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as s:
                while self._running:
                    ...
                    # --- START FIX: Handle exception on read ---
                    try:
                        data = self.input_stream.read(CHUNK, exception_on_overflow=False)
                    except (IOError, OSError) as e:
                        if self._running: # If we're not stopping, it's a real error
            ...
            # --- START FIX 2: Do not terminate p_in here ---
    ...
    def receive_audio(self):
        ...
        try:
            ...
            while self._running:
                try:
                    # --- START FIX: Use instance variable ---
                    ...
                    # --- END SYNTAX FIX ---
                    continue
                # --- START FIX: Handle socket close exception ---
            ...
            # --- START FIX: Use instance variables and clean up socket ---
            if self.output_stream and self.output_stream.is_active():
                self.output_stream.stop_stream()
                self.output_stream.close()
            # --- START FIX 2: Do not terminate p_out here ---
    ...
    # --- START FIX 2: New stop method to unblock threads ---
    def stop(self):
        self._running = False
        self.stop_file_playback()

        # Actively terminate PyAudio instances and close socket
        # This will raise exceptions in the blocking calls (read/recvfrom)
        # and allow the threads to exit cleanly.
        try:
            ...
            if hasattr(self, 'p_in'):
                self.p_in.terminate()
            if hasattr(self, 'p_out'):
                self.p_out.terminate() # <-- This unblocks the output stream
        except Exception as e:
            # Don't emit log_message here, worker might be shutting down
...
class StreamingTab(QWidget):
    ...
    def stop_streaming(self):
        ...
        if self.streamer_worker:
            ...
            self.streamer_worker.stop()
            self.streamer_thread.quit()
            self.streamer_thread.wait() # This is the line that hangs
            self.streamer_worker = None
    ...
    # --- START FIX 3: Add clean shutdown method ---
    def stop_all_threads(self):
        """Called on application close."""
        self.log("Stopping all background threads...")
        self.stop_streaming() # Stop audio stream if active
