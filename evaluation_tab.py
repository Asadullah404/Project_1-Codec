...
import shutil     # Added for checking ffmpeg
...
# --- FFMPEG Codec Classes (Copied from detailed_analysis_tab.py) ---
...
class FfmpegCodec:
    """Base class for codecs using ffmpeg for file-based processing."""
    def __init__(self, codec_name, bitrate_kbps, audio_format):
        ...
        self.format = audio_format # e.g., 'opus', 'aac'
        self.output_ext = f".{audio_format}"
        self.sr_str = str(SR)

    def process(self, original_wav_data, original_sr):
        """Encodes and decodes the audio data using ffmpeg via temp files."""
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                # 1. Write original audio to a temp .wav file
                orig_file = os.path.join(tmpdir, "original.wav")
                sf.write(orig_file, original_wav_data, original_sr)

                # 2. Define intermediate and final file paths
                encoded_file = os.path.join(tmpdir, f"compressed{self.output_ext}")
                recon_file = os.path.join(tmpdir, "reconstructed.wav")

                # 3. Build FFMPEG command for encoding
                encode_cmd = [
                    "ffmpeg", "-y", "-i", orig_file,
                    "-c:a", self.codec_name,
                    "-b:a", self.bitrate,
                    "-ar", self.sr_str, # Resample to target SR
                    "-ac", "1",
                    encoded_file
                ]
                subprocess.run(encode_cmd, capture_output=True, text=True, check=True)

                # 4. Build FFMPEG command for decoding
                decode_cmd = [
                ...
                # 5. Load the reconstructed .wav file
                reconstructed_wav_data, _ = librosa.load(recon_file, sr=SR, mono=True)
                return reconstructed_wav_data.astype(np.float32)

            except subprocess.CalledProcessError as e:
                print(f"FFMPEG Error ({self.codec_name}): {e.stderr}")
                raise RuntimeError(f"FFMPEG failed for {self.codec_name}: {e.stderr}")
            except Exception as e:
                print(f"File processing error: {e}")
                raise e

class OpusCodec(FfmpegCodec):
    def __init__(self, bitrate_kbps=12):
        super().__init__(codec_name="libopus", bitrate_kbps=bitrate_kbps, audio_format="opus")

class AacCodec(FfmpegCodec):
    def __init__(self, bitrate_kbps=12):
        super().__init__(codec_name="aac", bitrate_kbps=bitrate_kbps, audio_format="aac")
...
class EvaluationWorker(QObject):
    ...
    def run(self):
        ...
        try:
            ...
            # Resample to target SR (16000) for processing
            if original_sr != SR:
            ...
            with torch.no_grad():
                if self.model:
                    # --- ADDED: Handle FFMPEG codecs (file-based) ---
                    if isinstance(self.model, (OpusCodec, AacCodec)):
                        # These codecs process the *entire file* at once.
                        # We pass the original wav data and its *original* SR
                        # so ffmpeg can handle the resampling.
                        original_wav_for_ffmpeg, original_sr_for_ffmpeg = librosa.load(self.original_file_path, sr=None, mono=True)
                        reconstructed_wav = self.model.process(original_wav_for_ffmpeg, original_sr_for_ffmpeg)
                    
                    # --- EXISTING: Handle streaming codecs (OaS) ---
                    else:
            ...
            # For file-based codecs, RTF isn't a streaming metric
            if isinstance(self.model, (OpusCodec, AacCodec)):
                real_time_factor = processing_time / audio_duration # This is a file-processing factor, not streaming RTF
            ...
            results = {
                ...
                'is_file_based': isinstance(self.model, (OpusCodec, AacCodec))
            }
        except Exception as e:
            results['error'] = str(e)
            import traceback
...
class EvaluationTab(QWidget):
    ...
    def _setup_ui(self):
        ...
        # --- ADDED Bitrate SpinBox ---
        self.bitrate_spinbox = QSpinBox()
        self.bitrate_spinbox.setRange(8, 256) # 8kbps to 256kbps
        self.bitrate_spinbox.setValue(12)
        self.bitrate_spinbox.setSuffix(" kbps")
        self.bitrate_spinbox.setEnabled(False) # Disabled by default
        self.bitrate_spinbox.valueChanged.connect(lambda: self.on_model_type_changed(self.model_type_combo.currentText()))
        ...
        playback_layout.addStretch() # Add stretch to push download buttons to the right
        playback_layout.addWidget(self.download_original_button)
        playback_layout.addWidget(self.download_recon_button)
        layout.addLayout(playback_layout)

        # --- MODIFIED: Added ffmpeg check to status ---
        status_text = "Status: Ready."
        if not is_ffmpeg_available():
            status_text += " (WARNING: ffmpeg not found. Opus/AAC evaluation will fail.)"
    ...
    def on_model_type_changed(self, model_name):
        ...
        is_ffmpeg = any(x in model_name for x in ["Opus", "AAC"])
        
        # --- MODIFIED: Enable/Disable spinbox ---
        self.bitrate_spinbox.setEnabled(is_ffmpeg)
        
        self.model_path_edit.setEnabled(is_neural)
        self.browse_model_button.setEnabled(is_neural)
        
        if not is_neural and not is_ffmpeg:
            self.model_path_edit.setText("N/A (Traditional or Uncompressed)")
            if 'Uncompressed' in model_name:
                bitrate = '256 kbps'
                latency = '0ms (Streaming)'
            elif 'AMR-WB' in model_name:
                bitrate = '~12.65 kbps'
                latency = '20ms (Streaming)'
            else:
                bitrate = '128 kbps'
                latency = '15ms (Streaming)'
            self.bitrate_label.setText(f"Bitrate: {bitrate} | Latency: {latency}")
        
        # --- ADDED: Handle Opus/AAC ---
        elif is_ffmpeg:
            self.model_path_edit.setText("N/A (Using ffmpeg)")
            latency = 'File-based (Not Real-time)'
            # --- MODIFIED: Read bitrate from spinbox ---
            current_br = self.bitrate_spinbox.value()
            self.bitrate_label.setText(f"Bitrate: ~{current_br} kbps | Latency: {latency}")
            ...
            self.bitrate_label.setText(f"Bitrate: {BITRATE_TINY:.2f} kbps (Calculated) | Latency: 15ms (Streaming)")
    ...
    def load_model(self):
        ...
        # --- ADDED: Load Opus/AAC with bitrate ---
        elif "Opus" in model_type_str:
            if not is_ffmpeg_available():
                self.status_label.setText("Status: ERROR - ffmpeg not found. Please install ffmpeg.")
                return False
            bitrate_kbps = self.bitrate_spinbox.value() # Get bitrate from UI
            self.model = OpusCodec(bitrate_kbps=bitrate_kbps)
            self.status_label.setText(f"Status: Loaded Opus Codec ({bitrate_kbps} kbps via ffmpeg).")
            return True
        elif "AAC" in model_type_str:
            if not is_ffmpeg_available():
                self.status_label.setText("Status: ERROR - ffmpeg not found. Please install ffmpeg.")
                return False
            bitrate_kbps = self.bitrate_spinbox.value() # Get bitrate from UI
            self.model = AacCodec(bitrate_kbps=bitrate_kbps)
            self.status_label.setText(f"Status: Loaded AAC Codec ({bitrate_kbps} kbps via ffmpeg).")
            return True
    ...
    def run_evaluation(self):
        ...
        self.play_original_button.setEnabled(False)
        self.play_reconstructed_button.setEnabled(False)
        # --- START: Disable Download Buttons ---
        self.download_original_button.setEnabled(False)
        self.download_recon_button.setEnabled(False)
        # --- END: Disable Download Buttons ---
    ...
    def on_evaluation_complete(self, results):
        if results.get('error'):
            ...
            is_file_based = results.get('is_file_based', False)
            
            pesq_color = 'green' if pesq_score >= 3.5 else 'orange'
            stoi_color = 'green' if stoi_score >= 0.9 else 'orange'
            
            # --- MODIFIED: Handle RTF display for file-based vs streaming ---
            if is_file_based:
                rtf_color = 'gray'
                rtf_text = f"File-Processing Factor: {rtf_score:.3f}"
            else:
            ...
            if not is_file_based and rtf_score > 1.0:
